...
void Initialize(int& argc, char**& argv, unsigned random_seed) {
  ...
  //kSCALAR_MINUSONE = (float*)cnn_mm_malloc(sizeof(float), 512);
  *kSCALAR_MINUSONE = -1;
  kSCALAR_ONE = (float*) cnn_mm_malloc(sizeof(float), 256);
  //kSCALAR_ONE = (float*)cnn_mm_malloc(sizeof(float), 512);
  *kSCALAR_ONE = 1;
  kSCALAR_ZERO = (float*) cnn_mm_malloc(sizeof(float), 256);
  //kSCALAR_ZERO = (float*)cnn_mm_malloc(sizeof(float), 512);
  *kSCALAR_ZERO = 0;
#endif
  unsigned long num_mb = 2048;
  int argi = 1;
  while(argi < argc) {
